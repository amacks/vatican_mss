/* These SQL statements will add a fond_code column to the manuscripts table and set the 
   fonds.code column as an external constraint.  They must be run after the fonds table 
   has been created */
...
/* now that the 2 tables are setup with external constraints, populate manuscripts 
   where needed */

update manuscripts as m join fonds as f
	on m.shelfmark like concat(f.code, "%")
	set m.fond_code=f.code
	where m.fond_code is null